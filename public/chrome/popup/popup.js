...
export class ShowPatternButtons extends LitElement {
  ...
  fetch(chrome.runtime.getURL('data.tsv'))
    ...
    .then(data => {
      ...
      // Send a message to the content script with the list of words
      ...
      // Count patterns and update darkPatternsCount
      const patternCount = countPatterns(words);
      document.getElementById('darkPatternsCount').textContent = patternCount;
    });
  
  // Listen for a message with the count
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.count !== undefined) {
      // Update the darkPatternsCount element's text with the count
...
// List of known e-commerce sites
